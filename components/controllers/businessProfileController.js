...
exports.createBusinessProfile = async (req, res) => {
    try {
        const { mechanicId, mechanicType, location, servicesOffered, paymentMethod } = req.body;
        ...
        // accept certificate
        const documentCertificate = await cloudinary.uploader.upload(certificateFile[0].path, {
            folder: "autopadi",
            resource_type: "auto"
        });
...
exports.getAllBusinessProfiles = async (req, res) => {
    try {
        const businessProfiles = await BusinessProfile.find().sort({ createdAt: -1 });
        res.status(200).json({
            success: true,
